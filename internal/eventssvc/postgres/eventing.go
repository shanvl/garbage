...
func (e *EventingRepo) ChangePupilResources(ctx context.Context, eventID string, pupilID string,
	...
	_, err := e.db.Exec(ctx, changePupilResourcesQuery, pupilID, eventID, resources[eventssvc.Gadgets],
		resources[eventssvc.Paper], resources[eventssvc.Plastic])
...
func (e *EventingRepo) EventClasses(ctx context.Context, eventID string,
	...
	if filters.Name == "" {
		...
		// the event's date is needed when we create a eventssvc.Class from it's name.
		...
		if err := e.db.QueryRow(ctx, `select date from event where id = $1`, eventID).Scan(&eDate); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return nil, 0, eventssvc.ErrUnknownEvent
			}
			return nil, 0, err
		}
		// get the class' letter and date formed, using the event's date
		letter, dateFormed, err := eventssvc.ParseClassName(filters.Name, eDate)
	...
	for rows.Next() {
		...
		if eID.Status != pgtype.Present {
			return nil, total, eventssvc.ErrUnknownEvent
		}
...
func (e *EventingRepo) PupilByID(ctx context.Context, pupilID string,
	...
	if err != nil {
		...
		if errors.Is(err, eventssvc.ErrNoClassOnDate) {
...
func (e *EventingRepo) StoreEvent(ctx context.Context, event *eventssvc.Event) error {
