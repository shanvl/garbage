...
func TestEventingRepo_ChangePupilResources(t *testing.T) {
	...
	resources := eventsvc.ResourceMap{
		eventsvc.Plastic: 10,
		eventsvc.Paper:   15,
	}
	type args struct {
...
func TestEventingRepo_EventByID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "no event" && !errors.Is(err, eventsvc.ErrUnknownEvent) {
				t.Errorf("EventByID() want eventsvc.ErrUnknownEvent, got %v", err)
			}
		})
	}
}
...
func TestEventingRepo_StoreEvent(t *testing.T) {
	...
	event := &eventsvc.Event{
		...
		Name:             "some name",
		ResourcesAllowed: []eventsvc.Resource{eventsvc.Gadgets, eventsvc.Paper},
	}
	tests := []struct {
		name    string
		args    args
...
func TestEventingRepo_PupilByID(t *testing.T) {
	...
	eID, deleteE := createEvent(t, &eventsvc.Event{
		ID:               "someid",
		Date:             time.Now().AddDate(0, 1, 0),
		Name:             "some name",
		ResourcesAllowed: []eventsvc.Resource{eventsvc.Gadgets},
	})
	defer deleteE()

	pID, deleteP := createPupil(t, &eventsvc.Pupil{
		ID:        "someid",
		FirstName: "fn",
		LastName:  "sn",
	}, eventsvc.Class{
	...
	oldPID, deleteOldP := createPupil(t, &eventsvc.Pupil{
		ID:        "someanotherid",
		FirstName: "fn",
		LastName:  "sn",
	}, eventsvc.Class{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "no event" && !errors.Is(err, eventsvc.ErrUnknownEvent) {
				t.Errorf("PupilByID() error = %v, want eventsvc.ErrUnknownEvent", err)
			}
			if tt.name == "no pupil" && !errors.Is(err, eventsvc.ErrUnknownPupil) {
				t.Errorf("PupilByID() error = %v, want eventsvc.ErrUnknownPupil", err)
			}
		})
	}
}
...
func createEvent(t *testing.T, event *eventsvc.Event) (string, func()) {
	t.Helper()
...
func createPupil(t *testing.T, p *eventsvc.Pupil, c eventsvc.Class) (string, func()) {
	t.Helper()
	q := `
