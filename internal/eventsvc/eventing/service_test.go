...
func Test_service_DeleteEvent(t *testing.T) {
	...
	repository.DeleteEventFn = func(ctx context.Context, eventID string) error {
		if eventID == repoErrorEventID {
			return eventsvc.ErrUnknownEvent
		}
		return nil
	}
	s := eventing.NewService(&repository)
...
func Test_service_EventByID(t *testing.T) {
	...
	repository.EventByIDFn = func(ctx context.Context, id string) (event *eventing.Event, err error) {
		...
		return &eventing.Event{Event: eventsvc.Event{ID: id}}, nil
	}
	s := eventing.NewService(&repository)
	...
	tests := []struct {
		...
		},
		{
			...
			want:    &eventing.Event{Event: eventsvc.Event{ID: "123"}},
			wantErr: false,
		},
	}
	for _, tt := range tests {
...
func Test_service_ChangePupilResources(t *testing.T) {
	...
	resourcesBrought := eventsvc.ResourceMap{eventsvc.Plastic: 22}
	resourcesAllowed := []eventsvc.Resource{eventsvc.Plastic, eventsvc.Gadgets}
	ctx := context.Background()
	...
	repository.EventByIDFn = func(ctx context.Context, id string) (event *eventing.Event, err error) {
		return &eventing.Event{
				Event: eventsvc.Event{ID: id, ResourcesAllowed: resourcesAllowed},
			},
			nil
	}
	s := eventing.NewService(&repository)
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				eventID:   eventID,
				pupilID:   pupilID,
				resources: eventsvc.ResourceMap{eventsvc.Paper: 1},
			},
			wantErr: true,
		},
		{
			...
			args: args{
				...
				eventID:   eventID,
				pupilID:   pupilID,
				resources: eventsvc.ResourceMap{eventsvc.Paper: 11, eventsvc.Plastic: 33},
			},
			wantErr: true,
		},
		{
			...
			args: args{
				...
				eventID:   eventID,
				pupilID:   pupilID,
				resources: eventsvc.ResourceMap{eventsvc.Paper: -15, eventsvc.Plastic: 33},
			},
			wantErr: true,
		},
		{
			...
			args: args{
				...
				pupilID:   pupilID,
				resources: eventsvc.ResourceMap{eventsvc.Plastic: 11, eventsvc.Gadgets: 33},
			},
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.args.eventID == eventIDErrNoEventPupil && !errors.Is(err, eventsvc.ErrUnknownPupil) {
				t.Errorf("ChangePupilResources() on repo's ErrNoEventPupil didn't return eventsvc."+
